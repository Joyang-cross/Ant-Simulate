...
CREATE TABLE exchange_rate_daily (
	...
	base_currency varchar(3) NOT NULL, -- ex) USD
	quote_currency varchar(3) NOT NULL, -- ex) KRW
	
	rate_date date NOT NULL, -- 기준일
	rate numric(18, 6) NOT NULL, -- 환율
...
COMMENT ON TABLE exchange_rate_daily IS '환율 테이블(일자 기준)';

COMMENT ON COLUMN exchange_rate_daily.base_currency IS '기준 통화 (예: USD)';
COMMENT ON COLUMN exchange_rate_daily.quote_currency IS '변환 대상 통화 (예: KRW)';
COMMENT ON COLUMN exchange_rate_daily.rate_date IS '환율 기준 일자';
COMMENT ON COLUMN exchange_rate_daily.rate IS '해당 일자의 환율 값';
COMMENT ON COLUMN exchange_rate_daily.created_at IS '레코드 생성 시각';
